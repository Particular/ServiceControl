            ...
            using (var http = new HttpClient())
            {
                ...
                using (var response = await http.GetAsync(ravenUrl + "/license/status"))
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        ...
                        Assert.That(details.Expiration, Is.LessThan(DateTime.UtcNow.AddDays(14)), $"The RavenDB license expires {details.Expiration} which is less than 2 weeks. Contact RavenDB at <sales@ravendb.net> for the new license.");
                    });
                }
            }
        }
