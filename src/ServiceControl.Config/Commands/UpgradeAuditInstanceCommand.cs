        ...
        public override async Task ExecuteAsync(InstanceDetailsViewModel model)
        {
            ...
            if (!instance.AppConfig.AppSettingExists(AuditInstanceSettingsList.EnableFullTextSearchOnBodies.Name))
            {
                var dialogResult = windowManager.ShowYesNoCancelDialog(
                    ...
                    "ServiceControl Audit indexes message bodies to enable searching for messages by their contents in ServiceInsight. This has a performance impact on the ServiceControl Audit instance and the feature can be disabled if it is not required.", "Do you want to continue to support full text search for message bodies?", "YES", "NO");
