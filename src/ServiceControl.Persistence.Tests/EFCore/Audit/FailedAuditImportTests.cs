...
class FailedAuditImportTests : AuditRetentionTestBase
{
    IFailedAuditImportDataStore FailedAuditImportStore => ServiceProvider.GetRequiredService<IFailedAuditImportDataStore>();
    ...
    public async Task Stores_and_replays_a_failed_import()
    {
        ...
        Assert.That(await FailedAuditImportStore.QueryContainsFailedImports(), Is.True);
        ...
        Assert.That(await FailedAuditImportStore.QueryContainsFailedImports(), Is.False);
    }
    ...
    public async Task Spills_a_large_body_to_external_storage_and_replays_it()
    {
        var headers = WellFormedHeaders();
        var body = LargeBody();
        var externalId = FailedAuditImportEntity.ExternalBodyId(FailedAuditImport.DeriveKey(headers, "native-1"));
    ...
    public async Task A_failing_re_import_is_left_behind_while_the_rest_are_processed()
    {
        ...
        await FailedAuditImportStore.ProcessFailedAuditImports(
            (message, _) =>
            {
        ...
        Assert.That(secondRun.Select(message => message.Id), Is.EqualTo(new[] { "native-2" }));
    }

    [Test]
    public async Task Replays_across_multiple_pages()
    {
        await Store(Enumerable.Range(0, 250).Select(i => Import($"native-{i:D4}", BaseTime.AddSeconds(i))).ToArray());
    ...
    Task StoreImport(Dictionary<string, string> headers, byte[] body, string exceptionInfo = "boom", string nativeId = null)
    {
        ...
        return FailedAuditImportStore.StoreFailedAuditImport(new FailedAuditImport
        {
            Id = FailedAuditImport.DeriveKey(headers, nativeId).ToString(),
    ...
    static FailedAuditImportEntity Import(string nativeId, DateTime failedAt) => new()
    {
