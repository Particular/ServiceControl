namespace ServiceControl.Persistence.Tests.AuditCapable
{
    ...
    public class AuditCapableTestPersistenceConfiguration : IPersistenceConfiguration
    {
        ...
        public bool SupportsMaintenanceMode => CreateInnerConfiguration(PrimaryRootNamespace).SupportsMaintenanceMode;
        ...
        static IPersistenceConfiguration CreateInnerConfiguration(SettingsRootNamespace settingsRootNamespace)
        {
            ...
            var typeName = manifest.TypeName
                ?? throw new InvalidOperationException($"The persistence manifest for '{persistenceType}' names no configuration type.");

            var configurationType = Type.GetType(typeName, throwOnError: true)!;
