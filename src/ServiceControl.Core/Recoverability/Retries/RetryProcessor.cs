namespace ServiceControl.Recoverability.Retries
{
    ...
    class RetryProcessor : IWantToRunWhenBusStartsAndStops, IDisposable
    {
        ...
        void Stage(RetryBatch batch, IDocumentSession session)
        {
            //Clear Staging Queue
            noopRelocator.Run();
        ...
        void PutMessageInStagingQueue(StreamResult<MessageFailureHistory> current)
        {
            var failedMessage = current.Document;
