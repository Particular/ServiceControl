namespace ServiceControl.MessageFailures.Api
{
        ...
        IMessageActionAuditLog auditLog) : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> RetryMessageBy([FromQuery(Name = "instance_id")] string instanceId, string failedMessageId, CancellationToken cancellationToken = default)
        {
        ...
        [Route("errors/retry")]
        [HttpPost]
        public async Task<IActionResult> RetryAllBy(List<string> messageIds, CancellationToken cancellationToken = default)
        {
            ...
            await auditLog.AuditedOperation(user, MessageActionKind.Retry, Permissions.ErrorMessagesRetry, MessageActionScope.Batch,
                ...
                ct => messageSession.Send<RetryMessagesById>(m => m.MessageUniqueIds = messageIds.ToArray(), AuditHeaders.LocalSendOptions(user, operationId), ct), cancellationToken);
        ...
        [HttpPost]
        public async Task<IActionResult> RetryAllBy(string queueAddress, CancellationToken cancellationToken = default)
        {
            var user = userAccessor.Resolve(User);
            var operationId = this.AuditOperationId();
            await auditLog.AuditedOperation(user, MessageActionKind.Retry, Permissions.ErrorMessagesRetry, MessageActionScope.Queue,
                ...
                ct => messageSession.Send<RetryMessagesByQueueAddress>(m =>
                {
        ...
        [HttpPost]
        public async Task<IActionResult> RetryAll(CancellationToken cancellationToken = default)
        {
            var user = userAccessor.Resolve(User);
            var operationId = this.AuditOperationId();
            await auditLog.AuditedOperation(user, MessageActionKind.Retry, Permissions.ErrorMessagesRetry, MessageActionScope.All,
                ...
                ct => messageSession.Send(new RequestRetryAll(), AuditHeaders.LocalSendOptions(user, operationId), ct), cancellationToken);
        ...
        [HttpPost]
        public async Task<IActionResult> RetryAllByEndpoint(string endpointName, CancellationToken cancellationToken = default)
        {
            var user = userAccessor.Resolve(User);
            var operationId = this.AuditOperationId();
            await auditLog.AuditedOperation(user, MessageActionKind.Retry, Permissions.ErrorMessagesRetry, MessageActionScope.Endpoint,
                ...
                ct => messageSession.Send(new RequestRetryAll { Endpoint = endpointName }, AuditHeaders.LocalSendOptions(user, operationId), ct), cancellationToken);
