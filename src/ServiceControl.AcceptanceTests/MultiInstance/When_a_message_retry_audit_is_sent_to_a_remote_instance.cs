namespace ServiceBus.Management.AcceptanceTests.MultiInstance
{
    ...
    public class When_a_message_retry_audit_is_sent_to_a_remote_instance : AcceptanceTest
    {
        ...
        public async Task Should_mark_as_resolved_on_master()
        {
            ...
            await Define<MyContext>(Remote1, Master)
                ...
                .Done(async c =>
                {
                    ...
                    {
                        await IssueRetry(c, () => this.Post<object>($"/errors/{failure.UniqueMessageId}/retry", null, null, Master));
                        return false;
                    }
        ...
        Task<SingleResult<FailedMessage>> GetFailedMessage(MyContext c)
        {
            ...
            return this.TryGet<FailedMessage>("/errors/" + c.UniqueMessageId, msg => true, Master);
        }
