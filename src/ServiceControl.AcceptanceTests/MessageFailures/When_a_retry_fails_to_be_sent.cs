    ...
    public class When_a_retry_fails_to_be_sent : AcceptanceTest
    {
        ...
        public void SubsequentBatchesShouldBeProcessed()
        {
            ...
            Scenario.Define<MyContext>()
                ...
                .Done(ctx =>
                {
                    ...
                    return TryGetSingle("/api/errors/", out decomissionedFailure, m => m.Id == ctx.DecommissionedEndpointUniqueMessageId) &&
                           TryGetSingle("/api/errors/", out successfullyRetried, m => m.Id == ctx.MessageThatWillFailUniqueMessageId);
                })
        ...
        public class FailureEndpoint : EndpointConfigurationBuilder
        {
            ...
            public class MessageThatWillFailHandler: IHandleMessages<MessageThatWillFail>
            {
                ...
                {
                    Context.MessageThatWillFailUniqueMessageId = DeterministicGuid.MakeId(Bus.CurrentMessageContext.Id.Replace(@"\", "-"), Configure.EndpointName).ToString();
