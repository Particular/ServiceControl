        ...
        public async Task Only_unresolved_issues_should_be_retried()
        {
            ...
            await Define<MyContext>()
                ...
                .Done(async c =>
                {
                    ...
                    if (!c.ArchiveIssued)
                    {
                        var result = await this.TryGet<FailedMessage>("/errors/" + c.MessageToBeArchivedId, e => e.Status == FailedMessageStatus.Unresolved);
                    ...
                    if (!c.RetryAllIssued)
                    {
                        ...
                        var unresolvedResult = await this.TryGet<FailedMessage>("/errors/" + c.MessageToBeRetriedAsPartOfRetryAllId, e => e.Status == FailedMessageStatus.Unresolved);
                        ...
                        await this.Post<object>("/errors/retry/all");

                        return false;
                    }

                    var resolvedResult = await this.TryGet<FailedMessage>("/errors/" + c.MessageToBeRetriedAsPartOfRetryAllId, e => e.Status == FailedMessageStatus.Resolved);
                    ...
                    var archivedResult = await this.TryGet<FailedMessage>("/errors/" + c.MessageToBeArchivedId, e => e.Status == FailedMessageStatus.Archived);
