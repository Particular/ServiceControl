...
class When_mcp_server_is_enabled : AcceptanceTest
{
    ...
    [Test]
    public async Task Should_list_read_only_primary_instance_tools()
    {
        ...
        var sortedTools = mcpResponse.Result.Tools.OrderBy(tool => tool.GetProperty("name").GetString()).ToList();
        AssertReadOnlyTools(sortedTools);
    ...
    public async Task Should_call_get_errors_summary_tool()
    {
        ...
        {
            Assert.That(structuredContent.GetProperty("unresolved").GetInt64(), Is.GreaterThanOrEqualTo(0));
            Assert.That(structuredContent.GetProperty("archived").GetInt64(), Is.GreaterThanOrEqualTo(0));
            Assert.That(structuredContent.GetProperty("resolved").GetInt64(), Is.GreaterThanOrEqualTo(0));
            Assert.That(structuredContent.GetProperty("retryIssued").GetInt64(), Is.GreaterThanOrEqualTo(0));
        });
    }

    static void AssertReadOnlyTools(IReadOnlyCollection<JsonElement> tools)
    {
        ...
        Assert.That(names, Does.Contain("get_failed_message_by_id"));
        Assert.That(names, Does.Contain("get_failed_message_last_attempt"));
        Assert.That(names, Does.Contain("get_failed_messages_by_endpoint"));
        Assert.That(names, Does.Contain("get_failure_groups"));
        Assert.That(names, Does.Contain("get_failure_group_errors"));
        Assert.That(names, Does.Contain("get_retry_history"));
    }
