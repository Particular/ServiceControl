    ...
    : IAuditIngestionUnitOfWork
{
    ...
    static string BuildSearchableContent(Dictionary<string, string> headers, ReadOnlyMemory<byte> body)
    {
    ...
    static string? GetBodyAsText(Dictionary<string, string> headers, ReadOnlyMemory<byte> body)
    {
        if (body.IsEmpty)
        {
            return null;
