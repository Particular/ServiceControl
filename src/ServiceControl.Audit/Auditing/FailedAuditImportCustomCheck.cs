namespace ServiceControl.Audit.Auditing
{
    ...
    {
        public FailedAuditImportCustomCheck(SqlQueryStore store)
        ...
        {
            var failures = await store.FailedAuditImportsExist().ConfigureAwait(false);

            return failures ? CheckResult.Failed(message) : CheckResult.Pass;
        }

        readonly SqlQueryStore store;
