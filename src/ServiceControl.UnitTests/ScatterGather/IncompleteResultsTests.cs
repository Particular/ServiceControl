...
class IncompleteResultsTests
{
    ...
    [Test]
    public async Task A_remote_timeout_does_not_cut_the_slower_remotes_short()
    {
        var settings = Settings("http://audit-1/api", "http://audit-2/api", "http://audit-3/api", "http://audit-4/api");
        var factory = new FakeHttpClientFactory();
        var delay = TimeSpan.FromSeconds(2);
        factory.Register(settings.RemoteInstances[0], Status(HttpStatusCode.GatewayTimeout));
        factory.Register(settings.RemoteInstances[1], Delayed(delay, Healthy("msg-2")));
        factory.Register(settings.RemoteInstances[2], Delayed(delay, Healthy("msg-3")));
    ...
    static Func<HttpRequestMessage, CancellationToken, Task<HttpResponseMessage>> Delayed(TimeSpan delay, Func<HttpRequestMessage, CancellationToken, Task<HttpResponseMessage>> responder) =>
        async (request, token) =>
        {
