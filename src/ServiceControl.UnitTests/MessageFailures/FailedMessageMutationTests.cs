namespace Particular.Backend.Debugging.RavenDB.UnitTests
{
    ...
    [TestFixture]
    class FailedMessageMutationTests
    {
        [Test]
        public void RunMigration()
        {
            using (var store = InMemoryStoreBuilder.GetInMemoryStore())
            {
                ...
                store.DatabaseCommands.Put("FailedMessage", Etag.Empty, RavenJObject.Parse(OldFailedMessageDocument), new RavenJObject());
                ...
                {
                    var sagaHistories = session.Query<>()
                        .ToList();
