...
public class BatchPerMessageAuditTests
{
    ...
    [Test]
    public async Task ArchiveBatch_emits_one_message_entry_per_id_sharing_operation_id()
    {
        var audit = new RecordingMessageActionAuditLog();
        var controller = new ArchiveMessagesController(new TestableMessageSession(), null,
            new StubCurrentUserAccessor(new AuditUser("a", "a")), audit);

        await controller.ArchiveBatch(["m-1", "m-2", "m-3"]);
    ...
    [Test]
    public async Task Unarchive_ids_emits_one_message_entry_per_id_sharing_operation_id()
    {
        var audit = new RecordingMessageActionAuditLog();
        var controller = new UnArchiveMessagesController(new TestableMessageSession(),
            new StubCurrentUserAccessor(new AuditUser("a", "a")), audit);

        await controller.Unarchive(["m-1", "m-2", "m-3"]);
    ...
    [Test]
    public async Task RetryBy_ids_emits_one_message_entry_per_id_sharing_operation_id()
    {
        var audit = new RecordingMessageActionAuditLog();
        var controller = new PendingRetryMessagesController(new TestableMessageSession(),
            new StubCurrentUserAccessor(new AuditUser("a", "a")), audit);

        await controller.RetryBy(["m-1", "m-2", "m-3"]);
