...
public class EditFailedMessagesControllerAuditTests
{
    ...
    [Test]
    public async Task Edit_disabled_is_not_audited()
    {
        var audit = new RecordingMessageActionAuditLog();
        var store = new StubErrorMessageDataStore { ErrorByResult = new FailedMessage { ProcessingAttempts = { new FailedMessage.ProcessingAttempt() } } };

        await Create(store, audit, allowMessageEditing: false).Edit("msg-1", ValidEdit());

        Assert.That(audit.Operations, Is.Empty);
    }

    [Test]
    public async Task Edit_already_in_progress_is_not_audited()
    {
        var audit = new RecordingMessageActionAuditLog();
        var store = new StubErrorMessageDataStore
        {
            ErrorByResult = new FailedMessage { ProcessingAttempts = { new FailedMessage.ProcessingAttempt() } },
            EditManager = { CurrentEditingRequestId = "some-other-message-id" }
        };
    ...
    [Test]
    public async Task Edit_of_missing_message_is_not_audited()
    {
        var audit = new RecordingMessageActionAuditLog();
        var store = new StubErrorMessageDataStore { ErrorByResult = null };
    ...
    [Test]
    public async Task Edit_with_locked_header_violation_is_not_audited()
    {
        var audit = new RecordingMessageActionAuditLog();
        var store = new StubErrorMessageDataStore
        {
            ErrorByResult = new FailedMessage
            {
                ProcessingAttempts = { new FailedMessage.ProcessingAttempt { Headers = { [NServiceBus.Headers.MessageId] = "original" } } }
            }
        };

        var edit = new EditMessageModel { MessageBody = "body", MessageHeaders = new Dictionary<string, string> { [NServiceBus.Headers.MessageId] = "tampered" } };

        await Create(store, audit).Edit("msg-1", edit);

        Assert.That(audit.Operations, Is.Empty);
    }

    [Test]
    public async Task Edit_with_missing_body_is_not_audited()
    {
        var audit = new RecordingMessageActionAuditLog();
        var store = new StubErrorMessageDataStore { ErrorByResult = new FailedMessage { ProcessingAttempts = { new FailedMessage.ProcessingAttempt() } } };

        await Create(store, audit).Edit("msg-1", new EditMessageModel { MessageBody = "", MessageHeaders = [] });
